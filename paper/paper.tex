...
\title{Alignment Stability Under Progressive Data Corruption\\in Fine-Tuned Language Models}
...
\begin{figure}[htbp]
    \centering
    \fbox{\parbox{0.95\columnwidth}{\centering\vspace{0.3em}\textit{[Architecture / Pipeline diagram - to be inserted]}\vspace{0.3em}}}
    \caption{Experimental pipeline. Clean data from Stanford Alpaca (5K samples) is mixed with corrupted data at ratio $\rho$. Each mixture trains an independent LoRA adapter on \model{}. All models are evaluated across four dimensions: alignment, capability, drift, and geometry. Recovery models are produced by merging corrupted LoRA weights and training a fresh adapter on 2.5K clean samples.}
    \label{fig:pipeline}
\end{figure}
...
All stochastic metrics include bootstrap 95\% confidence intervals computed from 1,000 resamples. Non-linearity is quantified using the coefficient of variation (CV) of discrete derivatives: $\text{CV} = \sigma(\Delta S / \Delta \rho) / |\mu(\Delta S / \Delta \rho)|$, where CV $> 0.5$ indicates significant non-linearity.
...
This finding breaks down into three parts. \textbf{(i)} Recovery successfully brings toxicity down from 0.907 to 0.029, yet CKA similarity to baseline actually \emph{decreases} after recovery - the model has learned to produce safe outputs through a fundamentally different internal computation path. \textbf{(ii)} All three recovered models show perplexity around 12.5 to 12.7, well below the baseline's 28.9, suggesting overfitting to clean data patterns during recovery. \textbf{(iii)} KL divergence stays at 5.1 to 5.4 after recovery, pointing to a permanently shifted output distribution despite behavioral normalization.
